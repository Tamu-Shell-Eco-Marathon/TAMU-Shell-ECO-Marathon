...
COMP_ADJUST_TICKS_PER_MILE = (
    False  # Set True to also recalculate TICKS_PER_MILE on calibration
)
...
# ---- Button Timing ----
COMP_EXTRA_LONG_PRESS_MS = (
    5000  # 5 seconds for extra-long press to exit competition mode
)
...
class Vehicle:
    ...
    def update_states(self, dt, current_time_ms):
        ...
        if delta_ticks < 0:
            delta_ticks = 0  # Handle wrap/reset
        ...
        if eff_inst_raw > 999.0:
            eff_inst_raw = 999.0
        self.efficiency_instant = (self.EMA_ALPHA * eff_inst_raw) + (
            (1 - self.EMA_ALPHA) * self.efficiency_instant
        )

        # Update Timer & Distance
        if self.timer_running:
            self.timer_elapsed_seconds = (
                self._stored_elapsed_ticks
                + time.ticks_diff(current_time_ms, self._timer_start_ticks)
            ) / 1000
            ...
            eff_total_raw = (
                (self.distance_miles / energy_kwh) if energy_kwh > 0.0001 else 0.0
            )
            if eff_total_raw > 999.0:
                eff_total_raw = 999.0
            self.efficiency_total = (self.EMA_ALPHA * eff_total_raw) + (
                (1 - self.EMA_ALPHA) * self.efficiency_total
            )
        else:
            self.timer_elapsed_seconds = self._stored_elapsed_ticks / 1000

        # Update Target MPH
        self.remaining_distance_miles = max(
            self.GOAL_DISTANCE_MI - self.distance_miles, 0
        )
        self.remaining_time_seconds = max(
            self.GOAL_TIME_SEC - self.timer_elapsed_seconds, 0.001
        )
        self.target_mph = (
            (self.remaining_distance_miles / (self.remaining_time_seconds / 3600))
